...
sequence_length = 2000  # 8 regions * 10 words * 25 dimensional vectors + sentiment
cnn_epochs = 300
lstm_epochs = 300
batch_size = 1000
...
def get_split_prep_data():
    ...
    print("data.shape:")
    print(data.shape)     
    print(data[ data['sentiment'] == 0].size/(sequence_length+1))
    print(data[ data['sentiment'] == 2].size/(sequence_length+1))
    print(data[ data['sentiment'] == 1].size/(sequence_length+1))
...
def build_CNN_model():
    model = Sequential()
    model.add(Conv1D(filters=12, kernel_size=50, strides=25, border_mode='valid', input_shape=(sequence_length,1))) 
